...
function addFile(event) {
    ...
    if (phovid[1] === 'jpg' || phovid[1] === 'png') {
        ...
        $(".file_image_show").append('<div class="col-sm-4 mb-3 addremove' + addcount + '""><div class="crad text-center" id="file_empty" style="background:#efefef;"><photo class="image_align" src="' + tmppath + '" id="output" controls></photo><hr class="hr_margin"><div class="file_name remove_option">' + value + '</div><hr class="hr_margin">' +
            '<a class="btn edit_file_remove" onclick="removeadd(' + addcount + ')">Remove</a></div>');
    }
    if (1 == 1) {
        inputcount = 1 + addcount;
        $("#uploadInput").removeAttr("id");
        $(".add_input_file").append('<input type="file" class="addremove' + inputcount + '"" accept="image/png, image/gif, image/jpeg" name="photo[]" onchange="addFile(event)"  id="uploadInput"/>');
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data, function(key, val) {
                ...
                $(".edit_input_old").html(" ");
                $(".edit_input_old").append('<input type="file" name="newphoto[]" class="remove301" accept="image/png, image/gif, image/jpeg" onchange="editFile(event)"  id="uploadInputedit"/>');
                ...
                for (let i = 0; i < image.length; i++) {
                    ...
                    if (formate[1] === 'jpg' || formate[1] === 'png') {
                        count++;
                        $(".edit_input_file").append('<div class="col-sm-4 mb-3 remove10' + count + ' "><div class="crad text-center" style="background:#efefef;"><img class="image_align" src="gallery/' + image[i] + '"><hr class="hr_margin"><div class="file_name">' + image[i] + '</div><hr class="hr_margin">' +
                        ...
                        count++;
                        $(".edit_input_file").append('<div class="col-sm-4 mb-3 remove20' + count + '"><div class="crad text-center" style="background:#efefef;"><photo class="image_align" controls src="gallery/' + image[i] + '"></photo><hr class="hr_margin"><div class="file_name">' + image[i] + '</div><hr class="hr_margin">' +
...
function editFile(event) {
    ...
    if (phovid[1] === 'jpg' || phovid[1] === 'png') {
        ...
        $(".edit_input_file").append('<div class="col-sm-4 mb-3 remove30' + myFuncCalls + '"><div class="crad text-center" id="file_empty" style="background:#efefef;"><photo class="image_align" src="' + tmppath + '" id="output" controls></photo><hr class="hr_margin"><div class="file_name">' + value + '</div><hr class="hr_margin">' +
            '<a class="btn edit_file_remove" onclick="removeedit(30' + myFuncCalls + ')">Remove</a>' +
            '</div>');
    }
    ...
    $(".edit_input_old").append('<input type="file" class="remove30' + filecount + '" name="newphoto[]" accept="image/png, image/gif, image/jpeg", onchange="editFile(event)"  id="uploadInputedit"/>');
    // console.log(tmppath);
                ...
                columns: [{
                    ...
                    { data: 'description' },
                    {
                        ...
                        "render": function(data) {
                            ...
                            if (image_s.length >= 4) {
                                // console.log(image_s);
                                demo = '<span> <i class="bx bxs-right-arrow-circle"></i></span>'
                            } else {
                                demo = "";
                            }
                            var show = "";
                            ...
                            $.each(image_s, function(key, value) {
                                ...
                                show += '<li class="" style="display: inline-block; padding-left: 4px;">' +
                                    '<img style="width: 40px; height: 38px;"class="mani_m list-unstyled avatar  pull-up rounded-circle" src="gallery/' + mani + '" >' +
                                    '</li>'
                            });
                            fullvalu = show + " " + demo;
